...
contract("ZkpHealth", function(accounts) {
    ...
    describe("Public keys management with NFTs", async() => {
        ...
        it("should allow an authorized hospital with a valid ZKP token to add new public keys", async() => {
            ...
            const retrievedPublicKey1 = await zkpHealthInstance.getPublicKey(
                name,
                0
            );
            assert.equal(
            ...
            const retrievedPublicKey2 = await zkpHealthInstance.getPublicKey(
                name,
                1
            );
            assert.equal(
            ...
            const retrievedPublicKey3 = await zkpHealthInstance.getPublicKey(
                name,
                2
            );
            assert.equal(
            ...
            const retrievedPublicKey4 = await zkpHealthInstance.getPublicKey(
                name,
                3
            );
            assert.equal(
        ...
        it("should not allow an unauthorized hospital/user to set a public key", async() => {
            ...
            await expect(
                zkpHealthInstance.setPublicKey(
                    name,
                    helper.getRandomGrumpkinPublicKey(), { from: hospitalB }
                )
            ).to.be.rejectedWith(
