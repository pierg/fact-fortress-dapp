...
contract("ZkpContract", function (accounts) {
  ...
  beforeEach(async () => {
    ...
    zkpContractInstance = await ZkpContract.new(
  ...
  describe("Public keys management with NFTs", async () => {
    it("should allow an authorized hospital with a valid ZKP token to add their public key", async () => {
      ...
      await zkpContractInstance.setPublicKey(name, publicKey, {
        from: hospitalA,
      });

      // Verify that hospitalA's public key was set correctly
      const retrievedPublicKey = await zkpContractInstance.getPublicKey(
        name,
        0
      );
      assert.equal(
    ...
    it("should allow an authorized hospital with a valid ZKP token to add new public keys", async () => {
      ...
      await zkpContractInstance.setPublicKey(name, publicKey1, {
        from: hospitalA,
      });

      // Set hospitalA's second public key
      await zkpContractInstance.setPublicKey(name, publicKey2, {
        from: hospitalA,
      });

      // Set hospitalA's third public key
      await zkpContractInstance.setPublicKey(name, publicKey3, {
        from: hospitalA,
      });

      // Set hospitalA's fourth public key
      await zkpContractInstance.setPublicKey(name, publicKey4, {
        from: hospitalA,
      });

      // Verify that hospitalA's first public keys were set correctly
      const retrievedPublicKey1 = await zkpContractInstance.getPublicKey(
        name,
        0
      );
      assert.equal(
      ...
      const retrievedPublicKey2 = await zkpContractInstance.getPublicKey(
        name,
        1
      );
      assert.equal(
      ...
      const retrievedPublicKey3 = await zkpContractInstance.getPublicKey(
        name,
        2
      );
      assert.equal(
      ...
      const retrievedPublicKey4 = await zkpContractInstance.getPublicKey(
        name,
        3
      );
      assert.equal(
      ...
      const retrievedLatestPublicKey =
        await zkpContractInstance.getLatestPublicKey(name);
      assert.equal(
    ...
    it("should allow an hospital to update their own public key", async () => {
      ...
      await zkpContractInstance.setPublicKey(initialName, initialPublicKey, {
        from: hospitalA,
      });
      ...
      await zkpContractInstance.setPublicKey(updatedName, updatedPublicKey, {
        from: hospitalA,
      });
    ...
    it("should not allow an unauthorized hospital/user to set a public key", async () => {
      ...
      await expect(
        zkpContractInstance.setPublicKey(
          name,
      ...
      await expect(
        zkpContractInstance.getLatestPublicKey(name)
      ).to.be.rejectedWith(
    ...
    it("should not allow setting a public key with an empty string", async () => {
      ...
      const name = "Hospital A";
      await expect(
        zkpContractInstance.setPublicKey(name, "", { from: hospitalA })
      ).to.be.rejectedWith(
    ...
    it("should not allow an authorized hospital to modify another authorized hospital's public key with the same name", async () => {
      ...
      await zkpContractInstance.setPublicKey(
        name,
        helper.getRandomGrumpkinPublicKey()
      );
  ...
  it("should allow an authorized hospital to transfer their token and the new address to update their public key", async () => {
    ...
    await zkpContractInstance.setPublicKey(name, newPublicKey, {
      from: hospitalANewAddress,
    });

    // Verify that hospitalA's new public key was set correctly
    const retrievedNewPublicKey = await zkpContractInstance.getLatestPublicKey(
      name
    );
    assert.equal(
  ...
  describe("Verification of proofs", async () => {
    ...
    describe("simplified flow", async () => {
      it("signature stored", async () => {
        ...
        await zkpContractInstance.storeSignature(publicKey, signature, {
          from: hospitalA,
        });

        const signatureStored = await zkpContractInstance.checkSignature(
          publicKey,
          signature
        );
      ...
      it("valid proof of provenance", async () => {
        ...
        const publicKeyResult = await zkpContractInstance.verifyPublicInputsPoP(
          publicKey,
          proof
        );
        expect(publicKeyResult).eq(true);

        const smartContractResult = await zkpContractInstance.verifyProofPoP(
          proof
        );
      ...
      it("invalid proof of provenance", async () => {
        ...
        await expect(
          zkpContractInstance.verifyProofPoP(proof)
        ).to.be.rejectedWith(
